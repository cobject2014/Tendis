...
| `10.202.3.43` | Hyper-V full virtualization, Rocky Linux 9.7 | `16 vCPU`, `~23 GiB`, `Intel i9-13900KF` exposure, normal topology | `Msft Virtual Disk` via `storvsc`, write cache enabled | `dd bs=4K count=10000 oflag=dsync` -> `42.47s` on the earlier check, `39.00s` and `42.75s` on rerun | `118,624.07 ops/sec` | Small sync writes were still terrible, but the user-requested `SET p1` run completed and produced much higher throughput than the disk probe alone suggested. |
...
| `10.202.3.43` | `118,624.07` | `N/A` | `N/A` | [perf-amd64-native-hyperv-setp1-20260331.md](/Users/williamyao/dev/tendis_fork/build-arm/perf-amd64-native-hyperv-setp1-20260331.md) |
...
For write-heavy Tendis benchmarking, the small synchronous write path is still a useful quick filter, but `10.202.3.43` showed that it is not a perfect predictor of `SET p1` throughput by itself. The Hyper-V VM had a terrible `4K dsync` path yet still delivered the highest measured `SET p1` throughput among the three hosts. The VMware VM (`10.202.3.45`) was benchmarkable but storage-path-limited and topologically odd. The Aliyun ECS host (`39.106.7.212`) had the cleanest CPU presentation, but because the benchmark used the mounted root disk instead of the empty local NVMe, its write numbers still reflect significant disk-path cost rather than pure amd64 CPU capability.
